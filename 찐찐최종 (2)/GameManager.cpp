...
// 외부 스킬(Skill.cpp 등)에서 참조할 현재 전투 중인 바이옴
Biome* currentBattleBiome = nullptr;
...
GameManager::GameManager()
    : diceValue(0), moveCount(0), diceChance(10), isRunning(true), currentStage(1), map(), gameCleared(false) {
}

GameManager::GameManager(bool loadMode)
    : diceValue(0), moveCount(0), diceChance(10), isRunning(true), currentStage(1), map(loadMode), gameCleared(false) {
}
...
void GameManager::printGameScreen() const {
    ...
    rightLines.push_back("");
    rightLines.push_back("플레이어 HP: " + to_string(player.getHP()) + " / " + to_string(player.getMaxHP()));
...
void GameManager::handleTileEvent() {
    ...
    if (tile == 'M') {
        // 👇 [추가] 몬스터 전투 시작 전 현재 지형 장착
        currentBattleBiome = map.getBiome();
